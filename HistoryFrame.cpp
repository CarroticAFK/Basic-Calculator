...
HistoryFrame::HistoryFrame(const wxString& title) : wxFrame(NULL, wxID_ANY, "History", wxDefaultPosition, wxSize(400, 600))
{
	...
	historyTextCtrl = new wxTextCtrl(historyPanel, wxID_ANY, "", wxDefaultPosition, wxDefaultSize, wxTE_MULTILINE | wxTE_READONLY | wxTE_PROCESS_ENTER);
...
void HistoryFrame::saveHistory(const std::string& expression, const std::string& output)
{
	...
	historyTextCtrl->SetInsertionPointEnd();
	historyTextCtrl->SetSelection(0, 0);
}
